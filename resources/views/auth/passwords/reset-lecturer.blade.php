...
<head>
    ...
    <title>Premio | Lecturer Recover Password</title>
...
<section id="wrapper">
    <div class="login-register" style="background-image:url({{ asset('material-pro/assets/images/background/lock.jpg') }});">
        <div class="login-box card">
            <div class="card-body">
                <form class="form-horizontal form-material" id="loginform" action="{{ route('lecturer.password.request') }}" method="POST">
                    <h3 class="box-title m-b-20">Lecturer Recover Password</h3>
                    @csrf
                    <input type="hidden" name="token" value="{{ $token }}">
                    <div class="form-group">
                        <div class="col-xs-12">
                            <input name="email" value="{{ $email ?? old('email') }}" class="form-control" type="text" required="" placeholder="Email">
                            @if($errors->has('email'))
